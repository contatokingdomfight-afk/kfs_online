...
/**
 * Variante admin de /api/adesao/signature: usada quando o admin assina em nome de um aluno
 * sem sessão própria (ex.: sócio Kids) — /admin/alunos/[id]/contrato/assinar.
 * `studentId` vem no FormData (não há sessão de aluno para o resolver sozinho).
 */
export async function POST(request: Request) {
  ...
  if (!studentId) {
    return NextResponse.json({ error: "Aluno em falta." }, { status: 400 });
  }
  ...
  if (upErr) {
    console.error("admin adesao signature upload:", upErr);
