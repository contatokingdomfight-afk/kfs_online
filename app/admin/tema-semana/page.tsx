...
export default async function AdminTemaSemanaPage({ searchParams }: Props) {
  ...
  const allowedModalities = await getModalitiesForWeekThemeEditor(supabase, dbUser.role, null);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "clamp(20px, 5vw, 28px)", maxWidth: "min(720px, 100%)", width: "100%" }}>
      <div>
        ...
        <Link
          ...
          style={{ display: "inline-block", fontSize: "clamp(14px, 3.5vw, 16px)", color: "var(--primary)", fontWeight: 500, textDecoration: "none" }}
        >
          {t("weekThemeMonthlyEditThisWeekLink")} →
        </Link>
      </div>
      ...
      {allowedModalities.length > 1 && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
          {allowedModalities.map((modality) => (
            <Link
              key={modality}
              href={`/admin/tema-semana?month=${selectedMonth}&modality=${modality}`}
      ...
      >
        <Link
          href={`/admin/tema-semana?month=${prevMonth}&modality=${selectedModality}`}
          className="btn"
        ...
        <Link
          href={`/admin/tema-semana?month=${nextMonth}&modality=${selectedModality}`}
          className="btn"
          style={{ minHeight: 44, minWidth: 44, display: "inline-flex", alignItems: "center", justifyContent: "center", textDecoration: "none", fontSize: 18, color: "var(--text-primary)" }}
        >
          →
