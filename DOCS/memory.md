...
- **Cancelar aula (edição):** modal (portal em `document.body`) com **«Cancelar apenas esta aula»** ou **«Deletar esta e todas as futuras aulas a partir desta»**. Em sucesso, `deleteLesson` chama `redirect()` para `turmasPathAfterDelete` (`lib/turmas-list-query.ts`); o cliente trata `NEXT_REDIRECT` e tem fallback `window.location.assign`. Série: `series_future` com os mesmos critérios de antes (coach, escola, modalidade, horário, `isOpenClass`, dia da semana, `date` ≥ âncora).
