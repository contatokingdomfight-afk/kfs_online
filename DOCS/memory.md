...
- **Cancelar aula (edição):** modal (portal em `document.body`) com **«Cancelar apenas esta aula»** ou **«Deletar esta e todas as futuras aulas a partir desta»**. Em sucesso, `deleteLesson` devolve `{ redirectTo }` (sem `redirect()` na action — evita 400 Bad Request no cliente) e `CancelarAulaButton` faz `window.location.assign(redirectTo)`; `turmasPathAfterDelete` reconstrói query com `URLSearchParams`. Série: `series_future` com os mesmos critérios de antes.
