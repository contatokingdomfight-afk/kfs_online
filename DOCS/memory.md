...
- **Rodapé público:** `components/home/Footer.tsx` — morada e telefone da escola (`lib/school-contact.ts`, dados de `GYM_ENROLLMENT_INFO`); link `tel:` e Google Maps. Reutilizado em `/timer`, `/arbitragem`, `/termos` e `/privacidade` via `components/PublicSiteFooter.tsx`.
...
- **Modal «Avaliar performance» (baseline e reavaliação):** a **primeira** avaliação abre com **1** em todos os critérios (`DEFAULT_BASELINE` em `components/CoachStudentProfileModal.tsx`). Nas **reavaliações** o formulário é pré-preenchido com a última avaliação da modalidade (`lastEvalScoresByModality`), fornecido pelos três fluxos: `CoachAlunoEvaluateButton` (coach), `/admin/alunos/[id]` (admin) e roster de aula (`lib/coach-lesson-eligible-students.ts`). Ao guardar, persiste os critérios **tocados** na sessão **e** os **herdados** da última avaliação (retrato completo, não «compromete» valores não tocados); exige ≥1 critério tocado. O radar (`computeGeneralPerformanceScores` em `lib/performance-utils.ts`) calcula a média **só** com critérios presentes no JSON — critérios nunca avaliados não entram na conta (evita inflacionar a média com baseline artificial).
...
- **Presenças na aula sem RSVP (jul. 2026):** `/coach/aula` — roster de alunos elegíveis (`lib/coach-lesson-eligible-students.ts`), pesquisa e check-in manual (`coachCheckInStudent`). A elegibilidade usa `isLessonParticipationAllowedByPlan` (`lib/dashboard-lesson-filter.ts`): planos multi-modalidade só contam para as modalidades do plano (ex. MTKIDS não aparece para quem tem só Muay Thai/Boxing/MMA). Ver [`ESPECIFICACAO_DASHBOARD_COACH.md`](ESPECIFICACAO_DASHBOARD_COACH.md).
...
- **Prazos mensalidade:** pagamento até **dia 8** do mês; regularização até **15 dias corridos** após o dia 8 (`lib/lisbon-payment-dates.ts`, `paymentGraceEndsAt`; alterado de 5 dias úteis em ago. 2026). Ver [`PAGAMENTOS_MENSALIDADES_CRON.md`](PAGAMENTOS_MENSALIDADES_CRON.md).
...
- **Passo 1 — Comprovativo:** ficha de inscrição (`StudentEnrollmentForm`, migração `20260717150000_student_enrollment_form.sql`): layout oficial em **11 secções numeradas** (`components/documents/EnrollmentFormDocumentView.tsx` — ginásio, sócio, emergência, plano, modalidades com ☑, valores, pagamento, seguro PDCR, saúde, RGPD/consentimentos, declarações e assinaturas); CC/NIF, morada, emergência, saúde, consentimentos RGPD (sempre **sim** no comprovativo), forma de pagamento (**espécie** ou **transferência** para IBAN `LT383250045228499203` em `lib/enrollment-form.ts`; sem débito direto); cobertura PDCR (`lib/sports-insurance-coverage.ts`, `ENROLLMENT_INSURANCE_MANUAL_PLACEHOLDER=true` até fechar seguro — campos em branco; prémio APMDA 25 € quando a flag for `false`).
...
- **Consulta (só leitura):** `/dashboard/documentos-adesao` — comprovativo, condições gerais e termo em **secções colapsáveis** (`MembershipDocumentSection`); botão **Imprimir** sempre visível à direita; expandir para ler inline. **Imprimir** navega na mesma guia e abre o diálogo do browser (`AutoPrintTrigger` + `sessionStorage` `kfs-auto-print`; só dispara quando veio do botão). Ao guardar PDF, o browser sugere o nome **`Contrato de adesão {nome}`** ou **`Comprovativo de adesão {nome}`** via `document.title` (`PrintDocumentTitle`, `lib/print-document-title.ts`). Impressão em documentos separados: `/dashboard/documentos-adesao/imprimir/comprovativo` e `/imprimir/contrato` (`@page` com margem superior em todas as páginas; sem bordas/cabeçalho browser; chat oculto em `@media print` — `app/globals.css`). `/adesao` fica só para preencher/assinar; quem já assinou é redireccionado para documentos.
...
- Plano `plan-familia` (agrupamento na secretaria; **não** concede acesso MMA a todos — acesso vem do plano de referência de cada membro); **não** em `/escolher-plano` (nem cópias com nome «famil*»); descrição catálogo «a partir de 2 pessoas», sem tecto de membros.
...
- Mensalidade **combinada no titular** = soma dos planos de referência de cada membro − desconto % do grupo (`computeFamilyGroupMonthlyTuition`, `lib/family-tuition.ts`; fallback **80 €/pessoa** = `KINGDOM_PLAN_FAMILIA_MONTHLY_PER_PERSON` quando o membro não tem plano de referência). **Acesso individual** (modalidades, digital, check-in) vem do **plano de referência** de cada membro (`lib/family-effective-plan.ts`, `getPlanAccess`) — o `plan-familia` no `Student.planId` é só agrupamento/desconto, **não** «MMA para todos». Os membros **não** têm `TUITION` própria (só matrícula/seguro, individuais — `ensureOnboardingPendingPayments` salta a mensalidade para não-titulares); acesso com PAID individual (`lib/family-payment-gate.ts`).
...
- Overview `/admin/financeiro`: **`layout.tsx` + barra de tabs** (navegação entre sub-rotas sem grid de botões); link «Loja presencial» em linha própria no mobile (`FinanceiroTabs.module.css`); seletor de mês (`?month=AAAA-MM`); saldo = todas as receitas (breakdown + matrícula/seguro) − despesas; despesas com categoria; lista em **cartões** (mobile-first); modais dedicados (renovações, pagamentos, despesas, receitas, **depósito de espécie**) via `FinanceiroModals.tsx` — cabeçalho com **×**; modal de despesas com **filtros** (descrição, intervalo de datas, valor mín./máx.); ao remover despesa/depósito, `FormLoadingModal`; forma de pagamento padrão ao lançar = **TRANSFER**. Modal **Registos de pagamento**: editar/apagar por linha (`updateAdminPayment`, `deleteAdminPayment` em `actions.ts`; `EditPaymentForm`, `PaymentRecordActions`); **remover** pede confirmação num **modal próprio** (`ConfirmModal`, não o `confirm()` do browser); pacotes de 1.º pagamento apagam todos os IDs do bundle; pagamentos com `stripeInvoiceId` não editáveis; membros do plano família surgem como linha derivada a 0€ — **«Em atraso» com «Registar pagamento»** (reduz a combinada do titular) enquanto o titular não pagou, ou **«Pago» a 0€** (coberto pela família) depois de a titular pagar (ver secção Plano Família). Modal **Receitas por origem**: lista em cartões (`RevenueBreakdownList.tsx`).
...
- **Prazo de mensalidade:** regularização passou de **5 dias úteis** para **15 dias corridos** após o dia 8 (`lib/lisbon-payment-dates.ts`, `GRACE_PERIOD_DAYS`); `endOfGracePeriodLisbon` deixou de contar dias úteis (assume dia 8 + `GRACE_PERIOD_DAYS` ≤ 28, válido em qualquer mês). Ver [`PAGAMENTOS_MENSALIDADES_CRON.md`](PAGAMENTOS_MENSALIDADES_CRON.md).
- **Ordenação alfabética:** listas de alunos em `/admin/alunos`, `/coach/alunos` e presenças por aula em `/admin/presenca` passaram de ordenar por `createdAt`/hora de check-in para ordenar por **nome (A-Z)**. `/coach/aula` (roster de check-in) já ordenava por nome dentro de cada grupo de status — mantido como estava.
- **Lembrete de pagamento via WhatsApp:** `lib/whatsapp.ts` (normaliza telefone para `wa.me`, assume indicativo `+351` em números locais de 9 dígitos, mensagem padrão). Botão completo em `/admin/alunos/[id]` (só quando `status = INADIMPLENTE`); ícone compacto (💬, 40×40) ao lado de «Gerir pagamento» no painel «Ações imediatas» do dashboard admin (`app/admin/_components/ActionItems.tsx`, `lib/admin-action-items.ts` passou a trazer `studentPhone`).
- **Rascunho de módulos e aulas (biblioteca):** `CourseModule`/`CourseUnit` ganharam coluna `status` (`DRAFT`/`PUBLISHED`, migração `20260825120000_course_module_unit_status.sql`, default `PUBLISHED`). Admin (`/admin/cursos/[id]`) vê e edita tudo, com badge **RASCUNHO** e botão **▶ Pré-visualizar** (vídeo/texto inline, `ModuleCard.tsx`) — antes não havia forma de assistir nem de editar uma aula já criada. Aluno (`/dashboard/biblioteca`) e coach (`/coach/biblioteca`) só veem módulos/aulas `PUBLISHED`.
- **XP por assistir aulas:** `lib/course-watch-xp.ts` — **+5 XP** por aula concluída (`completeUnit`/`completeModule`), **+20 XP** de bónus ao completar 100% de um curso (tabela nova `CourseCompletion`, migração `20260825130000_course_completion_xp.sql`, evita conceder o bónus mais de uma vez). A subida de faixa continua **automática** (`syncAthleteDisplayBelt`, sem aprovação de coach/admin) — só passou a ser alimentada também por este XP. **Atenção:** a concessão de XP tem de usar o cliente **admin** (`createAdminClient`) — o cliente da sessão do aluno (RLS) não tem permissão de escrita em `Athlete.xp` e a escrita falha em silêncio.
- **Estatísticas de visualização (admin):** em `/admin/cursos/[id]`, cada aula mostra quantos alunos concluíram (`CourseUnitProgress`) e o curso mostra quantos completaram tudo (`CourseCompletion`), com drill-down por nome sob pedido (`app/admin/cursos/stats-actions.ts`, `ViewersDrilldown.tsx`). Não existe (ainda) visão equivalente para coach.
- **Tema da semana — modalidades dinâmicas:** `lib/coach-week-theme-modalities.ts` passou a ler o catálogo `ModalityRef` (gerido em `/admin/modalidades`) em vez de uma lista fixa de 4 códigos — modalidade nova já aparece em `/coach/tema-semana` sem alteração de código. Pendente: `MODALITY_LABELS` (`lib/lesson-utils.ts`) ainda não tem entrada para `MTKIDS`/`BKIDS` (mostra o código bruto se aparecerem aí).
- **Timer de rounds — áudio:** ver secção actualizada em [`ROUND_TIMER_COACH.md`](ROUND_TIMER_COACH.md) — reprodução via Web Audio API (não disputa foco com música de fundo), `ensureResumed()` antes de cada som (tablets suspendem o `AudioContext` sozinhos), fallback para `<audio>`, e sino a tocar mesmo com descanso a 0 s entre rounds (round→round colapsado no motor).
