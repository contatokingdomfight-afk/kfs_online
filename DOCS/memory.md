...
- **Admin — presença e KPIs (jul. 2026):** `/admin/presenca` e `lib/admin-dashboard-stats.ts` expandem ocorrências recorrentes via `expandLessonsForDateRange` (como `/coach/presenca` e Turmas); presenças filtradas por `Attendance.occurrenceDate`. KPI «Receita este mês» na home usa `getFinanceiroOverview` (alinhado com `/admin/financeiro`); gráfico receita 12 meses ignora `referenceMonth` inválido. Banner família: `getFamilyStudentBanner` lê nome do titular via service role (RLS bloqueava membros do grupo).
