...
- **Histórico:** combates `COMPLETED` com resultado oficial, secção expansível «Cartões dos juízes» (total, vencedor e placar por round); `loadFightJudgeHistory` em `lib/arbitration/queries.ts`.
- **Julgamento público:** `/julgamento` — ferramenta gratuita sem login (1 juiz, nomes editáveis, estado só no browser); link na homepage como o timer. Plataforma completa em `/coach/arbitragem`.
