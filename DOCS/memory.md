...
- **Ordem no `/dashboard` (aluno com plano):** «Sua próxima aula» (carrossel `OpenClassesCarouselShell` quando há várias ocorrências elegíveis em `primaryNextRows`) → **Painel do guerreiro** → «Nesta semana — aulas livres» (`openClassesSlot` em `DashboardBelowFold`, após `WarriorPanel`). Se **não** há próxima aula elegível (`primaryNextRows` vazio), o cartão vazio **não** aparece antes do guerreiro: o primeiro bloco do below-fold é o guerreiro. **Sem plano:** `DashboardBelowFold` não renderiza; o carrossel de aulas livres fica na `page.tsx` como antes.
...
- **Sessão (evitar logout após idle em mobile):** cookies Supabase com `cookieOptions` partilhado (`lib/supabase/cookie-options.ts`: `maxAge` ~400 dias, `secure` em produção); **middleware** e **`getCurrentDbUser`** usam só **`getUser()`** (valida/renova JWT e actualiza cookies — evita depender de `getSession()` só leitura). `components/AuthSessionKeepAlive.tsx` no `app/layout.tsx`: `startAutoRefresh()`, evento **`resume`** (Page Lifecycle / Android), `visibilitychange` / `pageshow` / `focus` / `online`, **`getUser()`** imediato + outro após ~400ms ao voltar à visibilidade; **intervalo ~45 min** enquanto o separador está **visível** para não deixar o JWT expirar só com timers internos pausados; `online` mantém throttle 45s. No Supabase Dashboard, **Auth → JWT expiry** (access token) e tempo de vida do refresh token afectam quanto tempo podes ficar offline sem novo login.
- **Tour no dashboard:** `StudentOnboardingGate` em `app/dashboard/layout.tsx`; wizard opcional em `app/onboarding/`.
- **Shell mobile (`components/ResponsiveShell.tsx` + `app/globals.css`):** em **viewport estreita**, `app/globals.css` fixa **`height: 100dvh`** e **`overflow: hidden`** em `.app-shell` / `.app-shell-inner` para o scroll ficar no **`main`** (sem isto o scroll era no documento e o efeito do header não disparava). O **header** fica `position: absolute`, **esconde** ao scroll para baixo (transição curta) e **reaparece de imediato** ao scroll para cima; no topo mantém-se visível; com o **drawer** aberto força-se visível. Desktop inalterado.
...
- **Notificações in-app (aluno):** tabela `Notification`; central `/dashboard/notificacoes`; `createInAppNotification` e tipos em `lib/notifications/in-app.ts`. **Nova avaliação do treinador:** tipo `COACH_EVALUATION` — criada após insert em `AthleteEvaluation` em `app/coach/atletas/actions.ts` (`createEvaluation`), `app/coach/aula/actions.ts` (`saveEvaluationFromLesson`) e `app/coach/alunos/[id]/actions.ts` (`saveStandaloneEvaluation`); `href` → `/dashboard/performance`.
...
- **Modalidade da aula:** o select vem de `ModalityRef` (ex. `MMA`, `BJJ`, `KRT`). A coluna `Lesson.modality` no Postgres é o enum **`Modality`** (Prisma). Migração `20260416180000_modality_enum_mma_bjj_krt.sql` adiciona `MMA`, `BJJ`, `KRT` ao enum para coincidir com `ModalityRef`; sem isso o insert falha com *invalid input value for enum "Modality"*.
