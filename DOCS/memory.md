...
- **Cancelar aula (edição):** modal (portal em `document.body`) com **«Cancelar apenas esta aula»** ou **«Deletar esta e todas as futuras aulas a partir desta»**. Lógica em `lib/admin/delete-lesson.ts` (`performDeleteLesson`). O botão chama **`POST /api/admin/turmas/delete-lesson`** (JSON) em vez da Server Action, para evitar 400 genérico do pipeline de actions; em sucesso, resposta JSON com `{ redirectTo }` e `CancelarAulaButton` faz `window.location.assign(redirectTo)`; `turmasPathAfterDelete` reconstrói query com `URLSearchParams`. A action `deleteLesson` em `app/admin/turmas/actions.ts` continua a delegar na mesma função se for chamada no servidor. Série: `series_future` com os mesmos critérios de antes.
