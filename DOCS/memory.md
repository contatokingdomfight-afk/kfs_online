...
- **Cancelar aula (edição):** modal (portal em `document.body`) com **«Cancelar apenas esta aula»** ou **«Deletar esta e todas as futuras aulas a partir desta»**. Lógica em `lib/admin/delete-lesson.ts` (`performDeleteLesson`). Antes do `DELETE` em `Lesson`, libertam-se referências em **`TrialClass.lessonId`** (FK sem `ON DELETE`), senão o Postgres bloqueia o apagar. O botão chama **`POST /api/admin/turmas/delete-lesson`**; autenticação nas rotas admin usa `getCurrentDbUserUncached()` (evita `cache()` do React com o contexto HTTP). **Guardar edição:** `POST /api/admin/turmas/update-lesson` (`lib/admin/update-lesson.ts` / `performUpdateLesson`), com validação coach↔escola; `EditarAulaForm` usa `fetch` em vez de Server Action. Em sucesso no cancelamento, `{ redirectTo }` e `window.location.assign(redirectTo)`; `turmasPathAfterDelete` reconstrói a query. Série: `series_future` com os mesmos critérios de antes.
