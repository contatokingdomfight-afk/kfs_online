...
- **Sessão (evitar logout após idle em mobile):** cookies Supabase com `cookieOptions` partilhado (`lib/supabase/cookie-options.ts`: `maxAge` ~400 dias, `secure` em produção); middleware chama `getSession()` antes de `getUser()` para alinhar refresh; `components/AuthSessionKeepAlive.tsx` no `app/layout.tsx` reage a `visibilitychange` / `pageshow` (bfcache) / `focus` / `online` e chama **`getUser()`** (renova JWT) ao voltar à visibilidade **sem** throttle de 45s nesse caso — só `online` mantém throttle; debounce ~400ms evita rajadas. No Supabase Dashboard, **Auth → JWT expiry** (access token) e tempo de vida do refresh token afectam quanto tempo podes ficar offline sem novo login.
