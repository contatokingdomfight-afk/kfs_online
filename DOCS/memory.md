...
- **Shell mobile (`components/ResponsiveShell.tsx`):** em **viewport estreita (breakpoint 768px)**, o scroll do conteúdo está no **`main`**; o **header** fica `position: absolute`, **esconde** ao scroll para baixo (transição curta) e **reaparece de imediato** ao scroll para cima; no topo do scroll o header mantém-se visível; com o **drawer** aberto o header força-se visível. Desktop inalterado.
