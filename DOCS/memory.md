...
- **Perfil do atleta (performance):** `/dashboard/performance` mostra médias agregadas dos check-ins com questionário (`PreLessonWellness`: sono, qualidade, hidratação, stress, fadiga, distribuição de zonas) em `CheckInWellnessSection` + `lib/check-in-wellness-aggregates.ts`; visível também quando ainda não há avaliações do coach mas já existem registos de pré-treino.
