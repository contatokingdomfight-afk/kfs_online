...
- O modelo é **escalado** de forma **uniforme** (ver `lib/humanoid-gltf-scene.ts`); **não** se aplica escala não uniforme a `SkinnedMesh`. Após o load, rotações discretas em **X/Z** endireitam GLBs «deitados»; depois **Euler Y** para frente; por fim um **refino em torno do Y mundial** (`rotateOnWorldAxis`) corrige perfil vs frente quando Rx/Rz ≠ 0. Linhas guia IK alinham-se ao diagrama 2D.
