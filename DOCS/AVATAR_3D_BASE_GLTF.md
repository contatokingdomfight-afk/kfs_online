...
- O modelo é **escalado** de forma **uniforme** (ver `lib/humanoid-gltf-scene.ts`); **não** se aplica escala não uniforme a `SkinnedMesh`. Após o load, rotações discretas em **X/Z** endireitam GLBs que chegam «deitados» (eixo longo em X ou Z); depois **Y** para o corpo ficar de frente à câmara. Linhas guia IK alinham-se ao diagrama 2D.
