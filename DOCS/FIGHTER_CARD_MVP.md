...
> **Estado:** implementado e migração aplicada em produção (confirmado por leitura directa a `information_schema.columns`) — **em produção, pronto a usar**. Verificado visualmente (dados de exemplo) que o cartão renderiza correctamente via `next/og`.
