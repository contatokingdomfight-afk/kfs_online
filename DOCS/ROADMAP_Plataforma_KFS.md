...
|---|------|-----------|--------|-------|
| 1 | **Cartão de conquista partilhável ("Fighter Card")** | Alunos | **Especificado — pronto para implementar** | Ver [`FIGHTER_CARD_MVP.md`](FIGHTER_CARD_MVP.md): imagem gerada no servidor (`next/og`), página pública `/t/f/[studentId]` (reaproveita o prefixo `/t/` já público), partilha WhatsApp + Instagram Stories via Web Share API, opt-in explícito (excluído para Kids), CTA no próprio cartão liga-se ao mecanismo de indicação já existente (`?ref=` → `referredByStudentId`) |
