...
| 7 | Crescimento (growth) | **Fighter Card partilhável**, **boletim dos pais (Kids)** e **perfil público do treinador** — as 3 ideias de maior interesse do brainstorm de crescimento; ver §16 para a lista completa (9 ideias, incl. retrospetiva semestral e feedback de técnica por IA) |
...
|---|------|-----------|--------|-------|
| 1 | **Cartão de conquista partilhável ("Fighter Card")** | Alunos | **Por fazer — próxima a especificar** | Imagem/página pública gerada automaticamente (faixa, XP, missões, sequência de presença, selo da escola), com botão de partilha nativa. Partilha para **WhatsApp**: link `https://wa.me/?text=...` (padrão já usado em `lib/whatsapp.ts`). Partilha para **Instagram Stories**: sem link universal — caminho é `navigator.share({ files: [imagem] })` (Web Share API nível 2, já usada em modo texto em `ReferralInviteSection.tsx`), que abre a folha de partilha nativa do telemóvel com o Instagram como opção; plano B (desktop / navegadores sem suporte a partilha de ficheiros) é botão de descarregar a imagem. Reaproveita o padrão de página pública `/t/p/[postId]` da Tribo (OG tags, landing para visitante sem conta) |
