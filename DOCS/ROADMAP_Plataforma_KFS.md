...
| Confirmação de conta / reset de password por email | Feito | Corrige erro no iPhone (link de confirmação a falhar) trocando PKCE por `token_hash`/`verifyOtp` — [`AUTH_EMAIL_CONFIRM.md`](AUTH_EMAIL_CONFIRM.md); templates "Confirm signup" e "Reset Password" trocados na Supabase e confirmado a funcionar em teste real no iPhone (25 set. 2026) |
...
| 2 | Subir o nível da indicação (`ReferralInviteSection`) | Alunos | **Especificado — por implementar** | Hoje é só link + contador. Ver [`REFERRAL_LEVELS_MVP.md`](REFERRAL_LEVELS_MVP.md): níveis de indicador, badge ao 3.º amigo convertido, leaderboard de quem mais trouxe gente. Reaproveita `grantReferralRewardIfEligible` já existente |
